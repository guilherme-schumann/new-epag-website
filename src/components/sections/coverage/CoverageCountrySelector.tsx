...
type Method = {
  label: string;
  type: string;
  icon: string | null; // path relative to /public, null = no asset available
};
      ...
      <div className="page-section page-container">
        ...
        <div className="mb-10 text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-base leading-7 text-dark-gray">
            Direct connections to domestic rails. No local entity required in any of the 12 markets.
          </p>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {selected.methods.map((method) => (
              <div
                key={method.label}
                className="flex items-center gap-3 rounded-2xl bg-light px-4 py-4 shadow-card"
              >
                {method.icon ? (
                  <Image
