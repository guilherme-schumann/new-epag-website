...
# Fetch Figma Design and Generate Component
...
2. **Fetch the design** using the Figma MCP tool with `fileKey` and `nodeId`.

3. **Treat the output as reference only** — do not copy it verbatim. Adapt to this project's conventions.
...
6. **Ask** which folder the component belongs to before writing:
